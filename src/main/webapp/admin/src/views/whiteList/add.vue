<template>
    <el-dialog :close-on-click-modal="false" title="添加商户至白名单" :visible="visible" @close="handleClose" width="30%">
        <el-form :model="form" label-position="top">
            <el-form-item label="商户注册手机号码 ：">
                <el-autocomplete
    ...
    export default {
        ...
        methods: {
            ...
            handleSubmit() {
                ...
                blackList.list({ merchantId: this.form.merchantId }).then(res => {
                    if (res.code === 100 && res.content && res.content.length > 0) {
                        this.$confirm('商户在黑名单中已经存在,继续添加将删除黑名单数据,是否继续添加？', '确认', {
                            ...
                            whiteList.insert(param).then(res => {
                                if (res.code === 100) {
                                    this.handleClose();
