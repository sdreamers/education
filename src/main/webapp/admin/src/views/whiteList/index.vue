            ...
            <el-row class="list-con clearfix">
                <el-row class="mb20" :gutter="20">
                    ...
                    <el-button style="float: right; margin-right: 20px;"
                               type="info"
                               round
                               v-if="G.permission['_account/whiteList/batchDelete']"
                               @click="handleRemoveMerchant">移除
                    </el-button>
                    <el-button style="float: right; margin-right: 10px;"
                               type="success"
                               round
                               v-if="G.permission['_account/whiteList/insert']"
                ...
                <el-table :data="records" border v-loading="loading" @select="handleSelectRow" @select-all="handleSelectAllRow">
                    ...
                    <el-table-column label="操作" width="150%" align="center">
                        <template slot-scope="scope">
                            <el-button
                                type="text"
                                v-if="G.permission['_account/whiteList/updateRemark']"
    ...
    export default {
        ...
        methods: {
            ...
            handlePagers() {
                ...
                whiteList.pagers(param).then(res => {
                    if (res.records) {
            ...
            handleRemoveMerchant() {
                if (!this.selectedRows || this.selectedRows.length < 1) {
                    return this.$notify.error('请至少选择一条白名单记录');
                }
                this.$confirm('删除以后将无法恢复，确定要删除吗？', '确认删除', {
                    ...
                    whiteList.batchDelete(param).then(res => {
                        if (res.code === 100) {
            ...
            handleSubmitEdit() {
                ...
                whiteList.updateRemark(param).then(res => {
                    if (res.code === 100) {
