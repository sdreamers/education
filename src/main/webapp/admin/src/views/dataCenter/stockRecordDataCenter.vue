...
<script>
    import api from '@/api/stockRecordDataCenter';
    import Util from '@/libs/util';
    import { warehouseByUserCurrentCity } from '@/api/dict';

    const columns = [
        { key: 'id', title: 'ID' },
        { key: 'projectId', title: '项目ID' },
        { key: 'warehouseName', title: '所属冻库' },
        { key: 'productId', title: '商品ID' },
        { key: 'productName', title: '商品名称' },
        { key: 'specificationId', title: '商品规格ID' },
        { key: 'specificationName', title: '商品规格' },
        { key: 'num', title: '数量' },
        { key: 'subTypeName', title: '库存变动类型' },
        { key: 'stockTime', title: '变动日期' }
    ];
        ...
        methods: {
            ...
            pagersData(isInitPage) {
                ...
                if (this.searchForm.subType) {
                    param.subType = this.searchForm.subType;
                }
                ...
                api.stockRecordDataCenterPagers(param).then(res => {
                    if (res.records) {
                        this.tableData = res.records.map(item => {
                            ...
                            item.warehouseName = this.warehouseList[item.warehouseId];
                            item.subTypeName = this.typeMap[item.subType];
