<template>
    <el-dialog :close-on-click-modal="false" title="添加 \ 修改 菜系类型" :visible="true" @close="$emit('close')">
        <el-form :model="form" :rules="rules" ref="ruleForm">
            ...
            <el-form-item label="菜系类型" :label-width="formLabelWidth" prop="name">
                ...
                if (valid) {
                    if (formType === 'edit') {
                        ...
                        if (param.name.length > 20) {
                            this.$notify.error('菜系类型不能大于20个字符');
                            return;
                        }
                        ...
                        api.productMerchantTypeInsert(param).then(res => {
                            if (res.code === 100) {
                                this.$notify.success(res.message || '成功');
                                this.$emit('close');
                            } else {
